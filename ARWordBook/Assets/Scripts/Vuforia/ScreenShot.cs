...
public class ScreenShot : MonoBehaviour
{
    ...
    {
#if UNITY_EDITOR 
        savePath = string.Format(@"C:\Users\강건\Desktop\db");
        StartCoroutine("Rendering");
        if (textLog_Text != null) textLog_Text.text = textLog;
        if (textSaveOption != 1) activePause = true;
#endif
#if UNITY_ANDROID
        savePath = string.Format(@"Galaxy S20+ 5G\Phone\Pictures");
        StartCoroutine("Rendering");
        if (textLog_Text != null) textLog_Text.text = textLog;
        if (textSaveOption != 1) activePause = true;
#endif
    ...
    IEnumerator Rendering()
    {
            ...
            if (savePath != null && !savePath.Equals(""))
            {
                ...
                switch (mode)
                {
                    case Mode.Screenshot:
                        ...
                        string path = savePath + @"\" +"ARBook "+ date + fileExt;
        ...
        {
            if (textSaveOption == 2 || textSaveOption == 0)
            { //When capture is over
                yield return new WaitForEndOfFrame();
    ...
    IEnumerator StopRecording(int localRecordNumber, string dirPath)
    {
        ...
        if (localRecordNumber == recordNumber)
    ...
    public void SaveTextLog(string name, bool printLog)
    { //For debug mode
        if (textLog != null && !textLog.Equals(""))
        { //Check contents blank
    ...
    private void SaveImageWithText(bool saveText)
    { //For Save Text Option
